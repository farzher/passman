...
async function decryptPayload(key, envelope) {
  try {
    ...
    if (!value || typeof value !== 'object' || !Number.isInteger(value.revision) || !Array.isArray(value.items) ||
        ...
        typeof value.deleted !== 'object' || value.deleted === null || Array.isArray(value.deleted)) throw new Error();
    return value;
  } catch {
...
function assertEnvelope(value) {
  try {
    ...
    if (e.passwordHint != null &&
        (typeof e.passwordHint !== 'object' || typeof e.passwordHint.text !== 'string' || e.passwordHint.text.length > 160 || !Number.isFinite(e.passwordHint.updatedAt))) throw new Error();
  } catch {
