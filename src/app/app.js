...
function shell(content, selected = 'passwords') {
  root.innerHTML = `<aside><div class="wordmark"><span>P</span>PassMan</div><nav><button data-go="" class="${selected === 'passwords' ? 'active' : ''}">Passwords</button><button data-go="settings" class="${selected === 'settings' ? 'active' : ''}">Settings</button></nav><button class="lock link">Lock</button></aside><main class="workspace">${content}</main>`;
...
async function settingsView() {
  ...
  shell(`<header class="page-head"><h1>Settings</h1></header><div class="settings">
    <section class="settings-card">
      ...
      <div class="setting-row">${icon('<circle cx="12" cy="12" r="8"></circle><path d="M12 8v4l3 2"></path>')}<div class="setting-copy"><h2>Auto-lock</h2></div><select class="autolock" aria-label="Auto-lock time"><option value="0">When Chrome closes</option><option value="15">After 15 minutes</option><option value="60">After 1 hour</option><option value="-1">Never this session</option></select></div>
    </section>
    <section class="settings-card">
